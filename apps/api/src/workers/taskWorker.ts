  ...
  try {
    ...
    const response = await fetch(`${RELAY_URL}/api/tasks/plan`, {
      ...
      body: JSON.stringify({
        ...
        acceptanceCriteria: task.acceptanceCriteria ? `<user_input>${task.acceptanceCriteria}</user_input>` : null,
        ...
        attachmentContext: attachmentContext ? `<user_input>${attachmentContext}</user_input>` : null,
