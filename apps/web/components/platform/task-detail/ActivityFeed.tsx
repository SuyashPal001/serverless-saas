        ...
        <div className="mt-8 border-t border-[#1e1e1e] pt-4">
            ...
            <div className="flex items-center gap-6 mb-6">
                <button
                    ...
                    className={cn("text-sm pb-2 transition-colors", activeTab === 'Thread' ? 'text-foreground border-b-2 border-white font-medium' : 'text-muted-foreground hover:text-foreground/80')}
                >
                    Thread
                </button>
                <button
                    onClick={() => setActiveTab('Timeline')}
                    className={cn("text-sm pb-2 transition-colors", activeTab === 'Timeline' ? 'text-foreground border-b-2 border-white font-medium' : 'text-muted-foreground hover:text-foreground/80')}
                >
                    Timeline
                </button>
            </div>
            ...
            {activeTab === 'Thread' && (
                <>
                    {comments.length === 0 ? (
                        <p className="text-sm text-muted-foreground/40 italic py-4">No messages yet. Be the first to leave a note.</p>
                    ) : (
            ...
            {activeTab === 'Timeline' && (
                events.length === 0 ? (
                    <p className="text-sm text-muted-foreground/40 italic py-4">No events yet.</p>
                ) : (
                    <div className="space-y-4 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-px before:bg-[#1e1e1e] ml-1">
                        {events.map(event => {
                            ...
                            if (event.eventType === 'status_changed') {
                                Icon = RefreshCw
                                description = `Status → ${event.payload?.to || 'unknown'}`
                            ...
                            return (
                                <div key={event.id} className="relative pl-9">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[11px] text-muted-foreground/30 ml-auto">{formatRelativeTime(event.createdAt)}</span>
                                    </div>
                                </div>
                            )
                        })}
